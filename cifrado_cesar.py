...
def ingresar_opcion(mensaje, opciones, numero_letras_respuesta):
    ...
    while bandera == 0:
        ...
        for char in opcion_escogida:
            if opciones.count(char) == 0:
                print( f"El caracter {char} no puede ser seleccionado" )
                caracteres_malos +=1
            elif len(opcion_escogida) != numero_letras_respuesta:
                caracteres_malos += 1
    ...
    for char in mensaje:
        if char != ' ':   #Para que no codifique los espacios
            ...
            if numero_codificado > 122 or (numero_codificado > 90 and numero_codificado < 97):
                numero_codificado -= 26     #Ya que las ultimas letras del abecedario de codifican desplazandose al inicio
...
if __name__ == '__main__':
    opcion = ingresar_opcion("Digite \"d\" si quiere decodificar un mensaje o \"c\" si lo quiere codificar ", "cd", 1)
    if opcion == 'c':
